...
<form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
 ...
 <div className='flex items-center justify-between flex-wrap'>
  <div className='flex flex-col gap-1'>
    ...
    type="number" placeholder='0' className='outline-none md:py-2.5
    py-2 px-3 rounded border border-gray-500' required/>
  </div>
...
<div className='relative'>
          ...
          {chapter.chapterContent.map((lecture, lectureIndex) => (
            <div key={lectureIndex} className='flex justify-between items-center mb-2'>
              <span>{lectureIndex + 1}. {lecture.lectureTitle} - {lecture.lectureDuration}
                mins - <a href={lecture.lectureUrl} target='_blank'
  ...
  cursor-pointer' onClick={() => handleChapter('add')}>+ Add Chapter</div>

  {/* POPUP - centré sans overlay */}
 {showPopup && (
  <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 
  bg-white border border-gray-300 shadow-xl p-4 rounded w-full max-w-80'>
